...
const ForumDiskusi = () => {
  ...
  const API_BASE_URL = 'http://localhost:3001/api/threads';
  ...
  const fetchCommentsForThread = async (threadId) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/${threadId}/comments`);
      setComments(prev => ({
        ...prev,
  ...
  const handleLike = async (threadId) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/${threadId}/like`, { action });
  ...
  const handleAddComment = async (threadId) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/${threadId}/comments`, {
  ...
  const handleAddThread = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(API_BASE_URL, newThreadForm);
      const newThread = {
          ...response.data,
