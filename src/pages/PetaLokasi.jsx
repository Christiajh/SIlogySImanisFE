...
const PetaLokasi = () => {
  ...
  return (
    <motion.div className="peta-lokasi-page-container" initial="hidden" animate="visible" variants={{
      hidden: { opacity: 0, y: 20 },
      visible: { opacity: 1, y: 0, transition: { duration: 0.8, ease: "easeOut" } }
    }}>
      <motion.h1 className="peta-lokasi-title-main" initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }}>
        <FaMapMarkedAlt className="title-icon" /> Peta Aksi Hijau Kita
      </motion.h1>

      <motion.div className="map-view-wrapper" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.3 }}>
        <MapView />
      </motion.div>

      <motion.section className="form-add-location-section" initial="hidden" animate="visible" variants={{
        ...
        {submitMessage && (
          <motion.div className={`submit-notification ${messageType}`} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
        ...
        <form onSubmit={handleSubmit} className="form-add-location-grid">
          ...
          <motion.button type="submit" className="btn-submit-location" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={isSubmitting || !form.name.trim()}>
            {isSubmitting ? (
              <>
